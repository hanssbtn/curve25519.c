...
def write_add_test():
	...
	r = (n + m) % ((2 ** 255) - 19)
	i = 0
	with open("tests/add_test.c", "w") as f:
		...
		for i in range(1, 100):
			...
			m = random.getrandbits(256) % ((2 ** 255) - 19)
			r = (n + m) % ((2 ** 255) - 19)
			f.write(f"\tk1 = (curve25519_key_t){{\n\t\t0x{(n) & bitmask:016X},\n\t\t0x{(n >> (64 * 1)) & bitmask:016X},\n\t\t0x{(n >> (64 * 2)) & bitmask:016X},\n\t\t0x{(n >> (64 * 3)) & bitmask:016X}\n\t}};\n")
			f.write(f"\tk2 = (curve25519_key_t){{\n\t\t0x{(m) & bitmask:016X},\n\t\t0x{(m >> (64 * 1)) & bitmask:016X},\n\t\t0x{(m >> (64 * 2)) & bitmask:016X},\n\t\t0x{(m >> (64 * 3)) & bitmask:016X}\n\t}};\n")
			f.write(f"\tk3 = (curve25519_key_t){{\n\t\t0x{(r) & bitmask:016X},\n\t\t0x{(r >> (64 * 1)) & bitmask:016X},\n\t\t0x{(r >> (64 * 2)) & bitmask:016X},\n\t\t0x{(r >> (64 * 3)) & bitmask:016X}\n\t}};\n")
...
def write_modulo_test():
	...
	n_mod = n % ((2 ** 255) - 19)
	i = 0
	with open("tests/modulo_test.c", "w") as f:
